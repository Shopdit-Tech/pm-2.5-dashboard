...
const CircularProgress = ({ value, max, size = 145, strokeWidth = 18, color = '#52c41a' }: {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      >
        <div className="text-xs text-gray-400 font-bold uppercase tracking-wide" style={{ marginBottom: isMobile ? 4 : 8, fontSize: isMobile ? '10px' : undefined }}>PM 2.5</div>
        <div className="font-bold" style={{ color, lineHeight: '1.2', fontSize: isMobile ? '30px' : '36px' }}>
          {value.toFixed(1)}
        </div>
        <div className="text-xs text-gray-400 font-medium" style={{ marginTop: isMobile ? 4 : 8, fontSize: isMobile ? '10px' : undefined }}>μg/m³</div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="flex gap-1.5 md:gap-2 w-full">
      {levels.map((level) => (
        <div
          key={level.level}
          className="flex-1 text-center rounded-lg font-bold text-white transition-all"
          style={{
...
const MetricCardWithBar = ({ label, value, unit, parameter }: {
  ...
  return (
    ...
    >
      <div className="text-gray-400 font-bold mb-1 uppercase tracking-wide" style={{ fontSize: '10px' }}>{label}</div>
      <div className="flex items-baseline gap-1 mb-2">
        <span className="text-lg md:text-xl font-bold" style={{ color }}>
          {value.toFixed(1)}
        </span>
        <span className="text-gray-400 font-medium" style={{ fontSize: '10px' }}>{unit}</span>
      </div>
      {/* Progress bar */}
      ...
      <div className="flex flex-col md:flex-row items-center md:items-center gap-4 md:gap-8 mb-6 md:mb-8">
        ...
        <div className="flex-shrink-0">
          <CircularProgress
            value={sensor.pm25}
            max={250}
            size={window.innerWidth < 768 ? 105 : 145}
            strokeWidth={window.innerWidth < 768 ? 14 : 18}
            color={pm25Color}
          />
        </div>
        ...
        <div className="flex-1 w-full md:w-auto">
          ...
          <div className="mb-4 md:mb-6">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wide" style={{ fontSize: '11px' }}>อุณหภูมิ</span>
            </div>
            <div className="text-2xl md:text-3xl font-bold text-gray-900" style={{ marginLeft: 2 }}>{sensor.temperature.toFixed(0)}°</div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wide" style={{ fontSize: '11px' }}>ความชื้น</span>
            </div>
            <div className="text-2xl md:text-3xl font-bold text-gray-900" style={{ marginLeft: 2 }}>{sensor.humidity.toFixed(0)}%</div>
          </div>
        </div>
      </div>

      {/* Location */}
      <div className="text-center mb-4 md:mb-6 py-3 md:py-4" style={{ borderTop: '1px solid #f3f4f6', borderBottom: '1px solid #f3f4f6' }}>
        <div className="flex items-center justify-center gap-2 text-gray-700">
          <EnvironmentOutlined style={{ fontSize: 12, color: '#667eea' }} />
          <span className="text-sm md:text-base font-bold" style={{ wordBreak: 'break-word', maxWidth: '90%' }}>{sensor.name}</span>
        </div>
      </div>
